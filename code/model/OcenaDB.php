...
class OcenaDB{
    ...
    public static function getByFilm($id){
        ...
        $stmt = $db->prepare("SELECT ocena.id, ocena.ocena, ocena.opis as opis, film.naslov, user.username
        FROM ocena
        INNER JOIN film ON ocena.filmID = film.id
        INNER JOIN user ON ocena.userID = user.id
    ...
    public static function insert($stOcena, $opisOcena, $userID, $filmID) {
        $db = DBInit::getInstance();

        $stmt = $db->prepare("INSERT INTO ocena (ocena, opis, userID, filmID) VALUES (:stOcena, :opisOcena, :userID, :filmID)");
        $stmt->bindParam(":stOcena", $stOcena);
        $stmt->bindParam(":opisOcena", $opisOcena);
